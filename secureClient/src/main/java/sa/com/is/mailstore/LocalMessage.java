...
public class LocalMessage extends MimeMessage {
    ...
    void populateFromGetMessageCursor(Cursor cursor) throws MessagingException {
       ...
       /* boolean signed = (cursor.getInt(24) == 1);
        boolean encrypted = (cursor.getInt(25) == 1);*/
        int columnIndex = cursor.getColumnIndex(StoreSchemaDefinition.SIGNED_COLUMN);
        if(columnIndex != -1)
        {
            boolean signingResult = (cursor.getInt(columnIndex) == 0) ? false : true;
