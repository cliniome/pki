...
import sa.com.is.service.MailService;
import sa.com.is.utils.FileUtils;
...
public class AccountSettings extends K9PreferenceActivity {
    ...
    private static final int PREFERENCE_FILE_PICKER_REQUEST = 10;
    public static final String KEY_PASSPHRASE="KEY";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if(keyPrefs != null)
        {
            keyPrefs.setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
                @Override
                public boolean onPreferenceChange(Preference preference, Object o) {

                    if(o == null ||o.toString().isEmpty()) return false;

                    SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(AccountSettings.this);

                    SharedPreferences.Editor editor = prefs.edit();

                    String PASS_PHRASE = KEY_PASSPHRASE+AccountSettings.this.mAccount.getEmail();

                    editor.putString(PASS_PHRASE, o.toString());
        ...
        filePickerPreference = (Preference)findPreference("pickACertificate");
        ...
        if(filePickerPreference != null)
        {
            ...
            String fileName = sharedPreferences.getString(this.mAccount.getEmail(),null);
            ...
            if(fileName != null)
            {
                ...
                try {
                    ...
                    displayFileName = FileUtils.getFileName(fileNameUri,this);

                }catch (Exception s)
                {
                    Log.e("AccountSettings",s.getMessage());
                }

                filePickerPreference.setSummary(displayFileName);
            }


            filePickerPreference.setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
                @Override
                public boolean onPreferenceClick(Preference preference) {
                   ...
                   if(Build.VERSION.SDK_INT < 19)
                   {
                       ...
                       startActivityForResult(Intent.createChooser(filePickerIntent, "Pick a Certificate")
                               , PREFERENCE_FILE_PICKER_REQUEST);
                   }else {
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if(requestCode == PREFERENCE_FILE_PICKER_REQUEST)
        {

           try {

               if(data == null) return;
               Uri filetoOpen = data.getData();

               if(filetoOpen == null) return;
               SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(this);
               SharedPreferences.Editor editor  = prefs.edit();

               editor.putString(this.mAccount.getEmail()
                       , filetoOpen.toString());
               editor.commit();
               String currentFile = FileUtils.getFileName(filetoOpen,this);
               filePickerPreference.setSummary(currentFile);
